...
def main() -> None:
    ...
    writer.append_to_postgres(df=df_2013, db_name=DB_NAME, table_name=TABLE_NAME, connection_string=CONNECTION_STRING)  
    writer.append_to_postgres(df=df_2017, db_name=DB_NAME, table_name=TABLE_NAME, connection_string=CONNECTION_STRING)
    writer.append_to_postgres(df=df_2019, db_name=DB_NAME, table_name=TABLE_NAME, connection_string=CONNECTION_STRING)
    writer.append_to_postgres(df=df_2020, db_name=DB_NAME, table_name=TABLE_NAME, connection_string=CONNECTION_STRING)
